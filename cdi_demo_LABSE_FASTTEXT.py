...
def prune_associates(
    ...
    Important: associates are no longer removed just because they also appear in
    the tested pair list. This allows direct pair words, such as dog = pies, to
    remain as associate anchors when they appear in the associates section.
    """
...
def rank_matches(
    ...
    for word in source_words:
        if word in source_vecs:
            profile_cache[("source", word)] = source_assoc_vectors @ source_vecs[word]
    for word in target_words:
        if word in target_vecs:
            profile_cache[("target", word)] = target_assoc_vectors @ target_vecs[word]
...
def main() -> int:
    ...
    for en, pl_list in dictionary.items():
        english_vocab.add(en)
        polish_vocab.update(pl_list)
    ...
    print("Step 6: Building bilingual associate-vector pairs...", flush=True)
    en_assoc_list = []
    pl_assoc_list = []

    for en, pl_list in dictionary.items():
        if en not in en_vecs:
            continue
        available_pl = [pl_vecs[word] for word in pl_list if word in pl_vecs]
        if available_pl:
            pl_mean = l2_normalize(np.mean(available_pl, axis=0))
            if pl_mean is not None:
                en_assoc_list.append(en_vecs[en])
                pl_assoc_list.append(pl_mean)

    en_assoc = np.asarray(en_assoc_list, dtype=np.float32)
    pl_assoc = np.asarray(pl_assoc_list, dtype=np.float32)
