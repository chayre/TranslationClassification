...
- LaBSE, FastText, and Word2Vec are supported.
...
--------
    ...
    python cdi_demo_LANGUAGE_GENERIC.py --dict data/big-cdi-dict.txt --list-8 --embedding word2vec --score rc --profile-filter top-n --top-n 100
...
def parse_args(
    ...
    parser.add_argument(
        "--embedding",
        choices=["labse", "fasttext", "word2vec"],
        default="labse",
        help=(
            "Embedding backend. LaBSE uses SentenceTransformer; FastText and Word2Vec use "
            "text-format vector files from --left-vectors and --right-vectors. Default: labse."
        ),
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            f"Static-vector file for the left language ({languages.left_code}). Automatic defaults: "
            f"data/cc.{languages.left_code}.300.vec for FastText or "
            f"data/word2vec.{languages.left_code}.vec for Word2Vec."
        ),
    )
    parser.add_argument(
        "--right-vectors",
        default=None,
        help=(
            f"Static-vector file for the right language ({languages.right_code}). Automatic defaults: "
            f"data/cc.{languages.right_code}.300.vec for FastText or "
            f"data/word2vec.{languages.right_code}.vec for Word2Vec."
        ),
    )
    parser.add_argument(
    ...
    if args.embedding in {"fasttext", "word2vec"}:
        script_dir = Path(__file__).resolve().parent

        def find_default_vector(code: str, backend: str) -> Optional[Path]:
            code_variants = unique_preserve_order([code, code.casefold()])
            for candidate_code in code_variants:
                if backend == "fasttext":
                    filenames = [f"cc.{candidate_code}.300.vec"]
                else:
                    # The recommended NLPL Word2Vec model.txt files can be renamed to
                    # word2vec.<language-code>.vec and placed in the data directory.
                    filenames = [
                        f"word2vec.{candidate_code}.vec",
                        f"word2vec.{candidate_code}.txt",
                        f"{candidate_code}.word2vec.vec",
                        f"{candidate_code}.word2vec.txt",
                    ]
                for filename in filenames:
                    candidate = script_dir / "data" / filename
                    if candidate.exists():
                        return candidate
            return None

        default_left = find_default_vector(languages.left_code, args.embedding)
        default_right = find_default_vector(languages.right_code, args.embedding)
        ...
        if not args.left_vectors or not args.right_vectors:
            if args.embedding == "fasttext":
                expected_pattern = "data/cc.<language-code>.300.vec"
            else:
                expected_pattern = "data/word2vec.<language-code>.vec"
            parser.error(
                f"--embedding {args.embedding} requires both --left-vectors and --right-vectors "
                f"when automatic {expected_pattern} discovery does not find both files."
            )
...
def load_text_vectors_subset(path: Path, needed_tokens: set[str]) -> Dict[str, np.ndarray]:
    """Stream a text-format static-vector file and retain only requested tokens."""
    if not path.exists():
...
def load_embedding_backend(
    ...
    """Load LaBSE, FastText, or Word2Vec vectors for both dictionary languages."""
    ...
    direct_cosine_available = bool(args.allow_static_crosslingual_cosine)
    if direct_cosine_available and left_vecs and right_vecs:
        left_dim = len(next(iter(left_vecs.values())))
        right_dim = len(next(iter(right_vecs.values())))
        if left_dim != right_dim:
            raise ValueError(
                "Direct cross-language cosine was enabled, but the two vector files have "
                f"different dimensions ({left_dim} vs {right_dim}). Use --score rc without "
                "--allow-static-crosslingual-cosine, or provide aligned vectors with matching dimensions."
            )

    return left_vecs, right_vecs, direct_cosine_available
